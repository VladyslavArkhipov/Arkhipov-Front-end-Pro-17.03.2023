...
); //При клике на одну из кнопок КУПИТЬ обертка формы становится видимой
...
function savingInfo(e) {
  ...
  for (let i = 0; i < form.length - 3; i++) {
    //Отнимаю 3 от длины формы, так как добавляются кнопки к форме и поле доп информации
    if (form[i].value === "") {
...
function showOrders() {
    ...
    div.innerHTML = `
        ...
        <div class="small_info">
            ...
            <p class="price">${Number(
              ordersList[i].productData.product_price.slice(0, 3) *
                ordersList[i].userData.count
            )} USD</p>
            ...
            <div class="full_info invisible">
                <p>${ordersList[i].productData.product_info}</p>
                  ...
                  <p>Post office: <span>${
                    ordersList[i].userData.post_office
                  }</span></p>
                  <p>Pay: <span>${ordersList[i].userData.pay}</span></p>
                  <p>Count: <span>${ordersList[i].userData.count}</span></p>
                  <p>Details: <span>${
                    ordersList[i].userData.details === undefined
                      ? "Empty"
                      : ordersList[i].userData.details
                  }</span></p>
            </div>
          </div>
        `;
  ...
  const fullInfo = document.querySelectorAll(".full_info"); //Нахожу блок с полной информацией по заказу

  orderCard.forEach((element) => {
    element.addEventListener("click", (e) => {
      fullInfo[element.id].classList.toggle("invisible");
